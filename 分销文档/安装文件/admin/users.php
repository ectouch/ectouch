...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    u.office_phone, u.home_phone, u.mobile_phone , u.apply_sale ,u.apply_time ".
        " FROM " .$ecs->table('users'). " u LEFT JOIN " . $ecs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
    ...
    if ($row)
    {
        ...
        $user['apply_sale']     = $row['apply_sale'];
        $user['apply_time']     = local_date("Y-m-d H:i:s",$row['apply_time'] );
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $other['home_phone'] = isset($_POST['extend_field4']) ? htmlspecialchars(trim($_POST['extend_field4'])) : '';
    $other['mobile_phone'] = isset($_POST['extend_field5']) ? htmlspecialchars(trim($_POST['extend_field5'])) : '';
    if (!empty($_POST['apply_sale'])){
        $other['apply_sale'] =htmlspecialchars($_POST['apply_sale']);
        if ($other['apply_sale'] ==2){
            $other['user_rank'] = 100;
        }
    }
    $db->autoExecute($ecs->table('users'), $other, 'UPDATE', "user_name = '$username'");
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time, apply_sale ,apply_time ".
